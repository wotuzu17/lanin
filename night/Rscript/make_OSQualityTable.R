...
# script examines data in table optionstats and creates a table with entries of each stock symbol
# for each symbol, it is determined:
#  - the number of zero iv
...
Sys.setenv(TZ="UTC")
scriptname <- "make_OSQualityTable.R" # for logging
start.time <- Sys.time()
...
processAllSyms <- function(con, mySyms) {
  vt <- c(30,60,90,120,150,180,360)
  zeros <- c()
  for (i in 1:length(mySyms)) {
    TS <- getOS(con, mySyms[i])
    for (j in 1:length(vt)) {
      zeros[j] <- sum(TS[,sprintf("iv%dmean", vt[j])] == 0)
    }
    ind <- c(index(TS), NA)
    ...
    feedOSQualityToDb(con, mySyms[i], zeros, timegaps, notfrombegin, nottoend)
  }
}
...
availableSyms <- getSymsFromTbl(con, "optionstats")
mySyms <- availableSyms[grep("[a-zA-Z0-9.-]", availableSyms)]

REF <- getOS(con, referenceSym)
