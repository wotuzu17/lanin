...
library("DBI")
library("RMySQL")
library("R.matlab")  # for writing .mat files
...
outp.names <- c("buy", "neutral", "sell", "pos", "neg", "y")
...
write.table(meanStToDf(nlist$colmean, nlist$colsd),
            ...
            row.names=FALSE)
# also save this stuff as .mat file
writeMat(paste0(saveTo, "/", DateString, "_normTrainData.mat"),
         X=as.matrix(nlist$normdf[,inp.names]),
         y=as.matrix(nlist$normdf[,"y"]),
         mean=nlist$colmean,
         sd=nlist$colsd, 
         verbose=TRUE)
