...
J = 1/m*s + lambda/2/m*(sum(sum(t_1.^2)) + sum(sum(t_2.^2)));

Theta2_grad = [Theta2_grad(:,1)/m ((Theta2_grad(:,2:end)/m) + lambda/m*t_2)];
Theta1_grad = [Theta1_grad(:,1)/m ((Theta1_grad(:,2:end)/m) + lambda/m*t_1)];

% unvectorized calculation of cost function
% output of Layer 1
%Y1 = sigmoid(Theta1 * [ones(m, 1) X]');
%h = sigmoid(Theta2 * [ ones(1, size(Y1,2)); Y1 ]);
